...
export function isGeminiAvailable(): boolean {
    return !!GEMINI_API_KEY || !!GEMINI_API_KEY_2;
}
...
export async function generateTextWithGemini(
    ...
    for (let i = 0; i < apiKeys.length; i++) {
        ...
        const isPrimary = i === 0;
        try {
            console.log(`🔄 Generando con Gemini (${isPrimary ? 'PRIMARIA' : 'SECUNDARIA'}, modelo: ${TEXT_MODEL})...`);
            ...
            if (errorMsg.includes('429') || errorMsg.includes('quota') || errorMsg.includes('Quota')) {
                console.error(`⚠️ Cuota de Gemini ${isPrimary ? 'primaria' : 'secundaria'} excedida, intentando siguiente...`);
                continue;
            } else if (errorMsg.includes('400') && errorMsg.includes(' SAFETY')) {
                console.error('❌ Contenido bloqueado por safety filters');
                return null;
            } else {
                console.error(`❌ Error con Gemini ${isPrimary ? 'primaria' : 'secundaria'}: ${errorMsg}`);
